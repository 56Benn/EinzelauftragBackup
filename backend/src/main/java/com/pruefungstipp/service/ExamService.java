...
public class ExamService extends BaseService {
    ...
    public Exam updateExam(Long id, Exam examDetails) {
        try {
            ...
            recalculatePointsIfGraded(savedExam);
            return savedExam;
    ...
    private void recalculatePointsIfGraded(Exam exam) {
        if (exam.getGrades() == null || exam.getGrades().isEmpty()) {
            return;
        }
        List<Prediction> predictions = predictionRepository.findByExam(exam);
        for (Prediction prediction : predictions) {
            Long studentId = prediction.getStudent().getId();
            Double grade = exam.getGrades().get(studentId);
            if (grade == null) {
                continue;
            }
            if (prediction.getPrediction1() != null) {
                prediction.setPoints1(pointsCalculationService.calculatePoints(prediction.getPrediction1(), grade));
            }
            if (prediction.getPrediction2() != null) {
                prediction.setPoints2(pointsCalculationService.calculatePoints(prediction.getPrediction2(), grade));
            }
        }
